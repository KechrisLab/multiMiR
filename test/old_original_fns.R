...
search.multimir_old <- function(url = multimir_URL, 
                            query) {
    query  <- paste("query", query, sep = "=")
    query  <- paste(url, query, sep = "?")
    query  <- myurlencode_old(query)
    result <- RCurl::getURL(query)
...
multimir_dbCount_old <- function(url = multimir_URL) {
...
multimir_dbInfo_old <- function(url = multimir_URL) {
...
multimir_dbSchema_old <- function(schema.file = multimir_schema_url) {
...
multimir_dbTables_old <- function(url = multimir_URL) {
    res <- search.multimir_old(url = url, query = "SHOW tables")
    return(res)
}
...
get.multimir.cutoffs_old <- function(cutoff.file = multimir_cutoffs_url) {
...
get.multimir_old <- function(url = multimir_URL, 
														 org = "hsa", 
														 mirna = NULL, 
														 target = NULL,
														 disease.drug = NULL, 
...
#     }
#     return(result)
    lapply(c("mirecords", "mirtarbase", "tarbase"), function(x) {
               get.multimir.by.table_old(url = url, 
                                         org = org, 
                                         mirna = mirna, 
                                         target = target, 
                                         table = x) 
...
#     }
#     return(result)
		lapply(c("diana_microt", "elmmo", "microcosm", "miranda", "mirdb",
						 "pictar", "pita", "targetscan"), function(x) {
					   get.multimir.by.table_old(url = url, 
																			 org = org, 
																			 mirna = mirna, 
																			 target = target, 
...
#     }
#     return(result)
    lapply(c("mir2disease", "pharmaco_mir", "phenomir"), function(x) {
						 get.multimir.by.table_old(url = url, 
																			 org = org, 
																			 mirna = mirna, 
																			 target = target, 
...
get.multimir.by.table_old <- function(url = NULL, 
                                  ...
                                  mirna.table = "mirna", 
                                  target.table = "target") {
    multimir.tables <- setdiff(as.character(multimir_dbTables_old()[, 1]),
                               c("map_counts", "map_metadata", "metadata"))
    if (!table %in% multimir.tables) {
        stop(paste("Table", table, "does not exist!\n", "Please use",
                   "'multimir_dbTables_old()' to see a list of available tables.\n", 
            sep = " "))
    }
    cat("Searching", table, "...\n")
