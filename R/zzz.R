...
search.multimir <- function(url=multimir_URL, query) {
...
multimir_dbCount <- function(url=multimir_URL) {
    ...
    for (i in 2:ncol(res)) {res[,i] <- as.numeric(as.character(res[,i]))}
    return (res)
}
...
list.multimir <- function(x=c("mirna","gene","drug","disease"), url=multimir_URL) {
	...
	#q <- "SELECT DISTINCT(disease)FROM mir2disease UNION SELECT DISTINCT(disease) FROM phenomir"	# did not work for IP's outside of campus - split the query into two
...
# To parse the result from the multimir web server.
# Two tables should return. The first table (result[[1]]) is the summary. And
# the second table (result[[2]]) has the result in details.
parse.multimir <- function(HTML.result) {
	...
	cat("Request to multiMiR web server failed. There could be incorrect syntax in your query, or you are not connected to the internet. Alternatively the multiMiR web server at http://multimir.ucdenver.edu is temporarily down.\n")
    }
    return (result)
}
...
get.multimir <- function(url=multimir_URL, org='hsa', mirna=NULL, target=NULL, disease.drug=NULL, table='validated', predicted.cutoff=NULL, predicted.cutoff.type='p', predicted.site='conserved', summary=FALSE, add.link=FALSE) {
    result <- list()
    if (table %in% c('all','validated','predicted','disease.drug')) {
	if (table == 'predicted' | table == 'all') {
	    # search predicted miRNA-target tables
	    result[['predicted']] <- get.multimir.predicted(url=url, org=org, mirna=mirna, target=target, cutoff=predicted.cutoff, cutoff.type=predicted.cutoff.type, site=predicted.site)
	    if (add.link & !is.null(result[['predicted']])) {result[['predicted']] <- add.multimir.links(result[['predicted']], org)}
	}
	if (table == 'validated' | table == 'all') {
	    # search validated miRNA-target tables
	    result[['validated']] <- get.multimir.validated(url=url, org=org, mirna=mirna, target=target)
	    if (add.link & !is.null(result[['validated']])) {result[['validated']] <- add.multimir.links(result[['validated']], org)}
	}
	if (table == 'disease.drug' | table == 'all') {
	    # search miRNA-disease tables
	    result[['disease.drug']] <- get.multimir.disease(url=url, org=org, mirna=mirna, target=target, disease.drug=disease.drug)
	    if (add.link & !is.null(result[['disease.drug']])) {result[['disease.drug']] <- add.multimir.links(result[['disease.drug']], org)}
	}
    }else {
	# search an individual table
	result[[table]] <- get.multimir.by.table(url=url, org=org, mirna=mirna, target=target, disease.drug=disease.drug, table=table, predicted.cutoff=predicted.cutoff, predicted.cutoff.type=predicted.cutoff.type, predicted.site=predicted.site)
	if (add.link & !is.null(result[[table]])) {result[[table]] <- add.multimir.links(result[[table]], org)}
    }
...
get.multimir.predicted <- function(url=NULL, org='hsa', mirna=NULL, target=NULL, cutoff=NULL, cutoff.type='p', site='conserved') {
    if (is.null(mirna) & is.null(target)) {return (NULL)}
    result <- NULL
    for (table in c('diana_microt','elmmo','microcosm','miranda','mirdb','pictar','pita','targetscan')) {
	r <- get.multimir.by.table(url=url, org=org, mirna=mirna, target=target, table=table, predicted.cutoff=cutoff, predicted.cutoff.type=cutoff.type, predicted.site=site)
	result <- rbind(result, r)
    }
    return (result)
}
...
get.multimir.disease <- function(url=NULL, org='hsa', mirna=NULL, target=NULL, disease.drug=NULL) {
	...
	r <- get.multimir.by.table(url=url, org=org, mirna=mirna, target=target, table=table, disease.drug=disease.drug)
	result <- rbind(result, r)
    }
    return (result)
}
...
get.multimir.by.table <- function(url=NULL, org='hsa', mirna=NULL, target=NULL, table=NULL, disease.drug=NULL, predicted.cutoff=NULL, predicted.cutoff.type='p', predicted.site='conserved', mirna.table='mirna', target.table='target') {
    ...
    if ((is.null(mirna) & is.null(target) & is.null(disease.drug)) | is.null(table)) {return (NULL)}
	...
	q <- paste("SELECT m.mature_mirna_acc, m.mature_mirna_id, t.target_symbol, t.target_entrez, t.target_ensembl, i.experiment, i.support_type, i.pubmed_id FROM", mirna.table, "AS m INNER JOIN", table, "AS i INNER JOIN", target.table, "AS t ON (m.mature_mirna_uid=i.mature_mirna_uid AND i.target_uid=t.target_uid) WHERE", sep=" ")
	if (is.null(mirna)) {
	# only target as query
	    q <- paste(q, "(t.target_symbol IN", target, "OR t.target_entrez IN", target, "OR t.target_ensembl IN", target, ")", sep=" ")
	}else if (is.null(target)) {
	# only mirna as query
	    q <- paste(q, "(m.mature_mirna_acc IN", mirna, "OR m.mature_mirna_id IN", mirna, ")", sep=" ")
	}else {
	# both mirna & target as query
	    q <- paste(q, "(m.mature_mirna_acc IN", mirna, "OR m.mature_mirna_id IN", mirna, ") AND (t.target_symbol IN", target, "OR t.target_entrez IN", target, "OR t.target_ensembl IN", target, ")", sep=" ")
	}
	if (!is.null(org)) {
	    if (table == 'tarbase') {
	...
	if (org == 'rno' & table %in% c('diana_microt','pictar','pita','targetscan')) {return (NULL)} 
	q <- paste("SELECT m.mature_mirna_acc, m.mature_mirna_id, t.target_symbol, t.target_entrez, t.target_ensembl FROM", mirna.table, "AS m INNER JOIN", table, "AS i INNER JOIN", target.table, "AS t ON (m.mature_mirna_uid=i.mature_mirna_uid AND i.target_uid=t.target_uid) WHERE", sep=" ")
	...
	if (!is.null(org)) q <- paste(q, " AND m.org = '", org, "' AND t.org = '", org, "'", sep="")
...
#		    cut <- 0.5
	...
	if (predicted.cutoff.type == 'p') {
	    ...
	    if (is.null(predicted.cutoff)) {
		if (cutoffs[[name]][['count']] >= 300000) {score.cutoff <- cutoffs[[name]][['300000']]}
	    }else {
		if (predicted.cutoff < 10000) {
		    ...
		    warning(paste("Number predicted cutoff (predicted.cutoff) ", predicted.cutoff, " is larger than the total number of records in table ", table, ". All records will be queried.\n", sep=""))
		}else {
	...
	if (!is.na(score.cutoff)) {
		...
		q <- paste(q, "AND i.context_plus_score <=", score.cutoff, "ORDER BY i.context_plus_score", sep=" ")
	    }
	}
    }
	...
	if (table == 'mir2disease') {
	    if (is.null(mirna) & is.null(disease.drug)) {return (NULL)}
	    q <- paste("SELECT m.mature_mirna_acc, m.mature_mirna_id, 'NA' AS target_symbol, 'NA' AS target_entrez, 'NA' AS target_ensembl, i.disease AS disease_drug, CONCAT_WS('. ', i.year, i.title) AS paper_pubmedID FROM", mirna.table, "AS m INNER JOIN", table, "AS i ON (m.mature_mirna_uid=i.mature_mirna_uid) WHERE", sep=" ")
	    if (!is.null(disease.drug)) {
	    ...
	    q <- paste("SELECT m.mature_mirna_acc, m.mature_mirna_id, t.target_symbol, t.target_entrez, t.target_ensembl, i.drug AS disease_drug, i.pubmed_id AS paper_pubmedID FROM", mirna.table, "AS m INNER JOIN", table, "AS i INNER JOIN", target.table, "AS t ON (m.mature_mirna_uid=i.mature_mirna_uid AND i.target_uid=t.target_uid) WHERE", sep=" ")
	    if (!is.null(target)) {
		q <- paste(q, "(t.target_symbol IN", target, "OR t.target_entrez IN", target, "OR t.target_ensembl IN", target, ")", sep=" ")
		cond <- cond + 1
	    }
	    ...
	    if (is.null(mirna) & is.null(disease.drug)) {return (NULL)}
	    q <- paste("SELECT m.mature_mirna_acc, m.mature_mirna_id, 'NA' AS target_symbol, 'NA' AS target_entrez, 'NA' AS target_ensembl, i.disease AS disease_drug, i.pubmed_id AS paper_pubmedID FROM", mirna.table, "AS m INNER JOIN", table, "AS i ON (m.mature_mirna_uid=i.mature_mirna_uid) WHERE", sep=" ")
	    if (!is.null(disease.drug)) {
		q <- paste(q, "(i.disease IN", disease.drug, "OR i.disease_class IN", disease.drug, ")", sep=" ")
		cond <- cond + 1
	    }
	    if (!is.null(org)) {
		if (cond == 0) {
	...
	if (!is.null(mirna)) {
		...
		q <- paste(q, "AND (m.mature_mirna_acc IN", mirna, "OR m.mature_mirna_id IN", mirna, ")", sep=" ")
	    }
	    cond <- cond + 1
	}
    }
    ...
    # query the database
    result <- search.multimir(url=url, query=q)
    if (!is.null(result)) result <- cbind(database=table, result)
    if (table %in% c('mir2disease','pharmaco_mir','phenomir')) result <- unique(result)
...
multimir.summary <- function(result, pair.index=2:6, order.by='all.sum') {
    ...
    info <- table(apply(r[,1:len], 1, function(x) {paste(x,collapse="|")}), r[,len+1])
    info.ncol <- ncol(info)
    if (info.ncol > 1) {
	all.sum <- apply(info, 1, function(x) {sum(x>0)})
	cols <- colnames(info)
	p.m <- match(cols, c('diana_microt','elmmo','microcosm','miranda','mirdb','pictar','pita','targetscan'))
	if (sum(!is.na(p.m)) > 1) {
	    p.sum <- apply(matrix(info[,!is.na(p.m)], ncol=sum(!is.na(p.m))), 1, function(x) {sum(x>0)})
	...
	if (sum(!is.na(v.m)) > 1) {
	    v.sum <- apply(matrix(info[,!is.na(v.m)], ncol=sum(!is.na(v.m))), 1, function(x) {sum(x>0)})
	...
	if (sum(!is.na(d.m)) > 1) {
	    d.sum <- apply(matrix(info[,!is.na(d.m)], ncol=sum(!is.na(d.m))), 1, function(x) {sum(x>0)})
    ...
    n.s <- ncol(s)
    n.i <- ncol(info)
    for (n in (n.s-n.i+1):n.s) {s[,n] <- as.numeric(as.character(s[,n]))}

    return (s)
}


# To change the encoding of URL (to account for the OS difference).
# This is modified from the URLencode function in the utils package.
...
add.multimir.links <- function(x, org) {
	...
	if (d == 'mirecords') {
	    ...
	    if (org == 'hsa') {s <- "species=Homo+sapiens"
	    }else if (org == 'mmu') {s <- "species=Mus+musculus"
	    }else if (org == 'rno') {s <- "species=Rattus+norvegicus"}
	    links[m] <- paste("http://mirecords.biolead.org/interactions.php?", s, "&mirna_acc=", mir, "&targetgene_type=symbol&targetgene_info=", symbol, "&v=yes&search_int=Search", sep="")
	}else if (d == 'mirtarbase') {
	    symbol <- as.character(x$target_symbol[m])
	    links[m] <- paste("http://mirtarbase.mbc.nctu.edu.tw/php/search.php?org=", org, "&mirnas=", mir, "&targets=", symbol, "&opt=adv", sep="")
	}else if (d == 'tarbase') {
	    symbol <- as.character(x$target_symbol[m])
	    links[m] <- paste("http://diana.imis.athena-innovation.gr/DianaTools/index.php?r=tarbase/index&mirnas=", mir, "&genes=", symbol, sep="")
	}else if (d == 'mir2disease') {
	    # NOTE: Can only search by miRNA, gene or disease alone
	    # - here use gene
	    symbol <- as.character(x$target_symbol[m])
	    links[m] <- paste("http://watson.compbio.iupui.edu:8080/miR2Disease/searchTarget.jsp?SearchUnit=target&SearchText=", symbol, "&checkbox2=Causal&checkbox2=Unspecified", sep="")
	    ...
	    # NOTE: search by gene
	    symbol <- as.character(x$target_symbol[m])
	    links[m] <- paste("http://mips.helmholtz-muenchen.de/phenomir/main/list/searchform2?query=", symbol, "&selectedview=mirs&searchtype=fuzzy", sep="")
	}else if (d == 'diana_microt') {
	    ensembl <- as.character(x$target_ensembl[m])
	    links[m] <- paste("http://diana.imis.athena-innovation.gr/DianaTools/index.php?r=microT_CDS/results&genes=", ensembl, "&mirnas=", mir, "&threshold=0", sep="")
	}else if (d == 'elmmo') {
	    # NOTE: Need RefSeq accession for the gene - use miRNA only
	    mir <- sub("-5p", "", mir)
	    mir <- sub("-3p", "", mir)
	    if (org == 'hsa') {s <- "organism=hg"
	    }else if (org == 'mmu') {s <- "organism=mm"
	    }else if (org == 'rno') {s <- "organism=rn"}
	    links[m] <- paste("http://www.mirz.unibas.ch/ElMMo3/?", s, "&cellType=all&miRNAs[]=", mir, "&predict=Predict+miRNAs+targets+!", sep="")
	}else if (d == 'microcosm') {
	    mir <- sub("-5p", "", mir)
	    mir <- sub("-3p", "", mir)
	    if (org == 'hsa') {s <- "genome_id=2964"
	    }else if (org == 'mmu') {s <- "genome_id=3876"
	    }else if (org == 'rno') {s <- "genome_id=5171"}
	    symbol <- as.character(x$target_symbol[m])
	    links[m] <- paste("http://www.ebi.ac.uk/enright-srv/microcosm/cgi-bin/targets/v5/hit_list.pl?", s, "&mirna_id=", mir, "&external_name=", symbol, sep="")
	}else if (d == 'miranda') {
	    # NOTE: Could only search by gene or miRNA - use gene here
	    if (org == 'hsa') {s <- "organism=9606"
	    }else if (org == 'mmu') {s <- "organism=10090"
	    }else if (org == 'rno') {s <- "organism=10116"}
	    symbol <- as.character(x$target_symbol[m])
	    links[m] <- paste("http://www.microrna.org/microrna/searchGenes.do?gene=", symbol, "&", s, sep="")
	}else if (d == 'mirdb') {
	    # NOTE: Could only search by gene or miRNA - use gene here
	    if (org == 'hsa') {s <- "species=Human"
	    }else if (org == 'mmu') {s <- "species=Mouse"
	    }else if (org == 'rno') {s <- "species=Rat"}
	    symbol <- as.character(x$target_symbol[m])
	    links[m] <- paste("http://mirdb.org/cgi-bin/search.cgi?", s, "&searchType=gene&geneChoice=symbol&searchBox=", symbol, sep="")
	}else if (d == 'pictar') {
	    # NOTE: Links don't work
	    ...
	    mir <- sub("-5p", "", mir)
	    mir <- sub("-3p", "", mir)
	    if (org == 'hsa') {s <- "Organism=Human"
	    }else if (org == 'mmu') {s <- "Organism=Mouse"}
	    symbol <- as.character(x$target_symbol[m])
	    links[m] <- paste("http://genie.weizmann.ac.il/cgi-bin/search_mir07_prediction.pl?", s, "&microRNAs=", mir, "&Genes=", symbol, "&MinimumSeed=7&AllowSingleGU=1&AllowSingleMismatch=1&MinConservation=0&FlankOption=0_0", sep="")
	}else if (d == 'targetscan') {
	    mir <- sub("-5p", "", mir)
	    mir <- sub("-3p", "", mir)
	    symbol <- as.character(x$target_symbol[m])
	    if (org == 'hsa') {
		links[m] <- paste("http://www.targetscan.org/cgi-bin/targetscan/vert_61/targetscan.cgi?species=Human&gid=", symbol, "&mirg=", mir, sep="")
	    }else if (org == 'mmu') {
		links[m] <- paste("http://www.targetscan.org/cgi-bin/targetscan/mmu_61/targetscan.cgi?species=Mouse&gid=", symbol, "&mirg=", mir, sep="")
	    }
	}
    }
