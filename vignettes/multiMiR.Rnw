...
databases and flexibility. The R package \Rpackage{multiMiR}, with web server at
\url{http://multimir.ucdenver.edu}, is a comprehensive collection of predicted
and validated miRNA-target interactions and their associations with diseases and
drugs. \Rpackage{multiMiR} includes several novel features not available in
existing R packages: 1) compilation of 14 different databases, more than any
other collection; 2) expansion of databases to those based on disease annotation
and drug response, in addition to many experimental and computational databases;
and 3) user-defined cutoffs for predicted binding strength to provide the most
confident selection. The \Rpackage{multiMiR} package enables retrieval of
miRNA-target interactions from 14 external databases in R without the need to
visit all these databases. Advanced users can also submit SQL queries to the web
...
\Rpackage{multiMiR} web server and its database. Information from each external
database is stored in a table in the \Rpackage{multiMiR} database. To see a list
of the tables, we can use the \Rfunction{multimir\_dbTables} function.
...
To display the database schema, we can use the \Rfunction{multimir\_dbSchema}
function.
...
The function \Rfunction{multimir\_dbInfo} will display information of external
miRNA and miRNA-target databases in \Rpackage{multiMiR}, including version,
...
\Rfunction{multimir\_dbCount} function.
...
\Rpackage{multiMiR} package also provides the \Rfunction{list.multimir} function
to list all the unique miRNAs, target genes, drugs, and diseases in the
\Rpackage{multiMiR} database.
...
The current version of \Rpackage{multiMiR} has 4210 miRNAs and 94792 target
...
\Rfunction{get.multimir} is the main function in the package to retrieve
predicted and validated miRNA-target interactions and their disease and drug
associations from the \Rpackage{multiMiR} database.

To get familiar with the parameters in \Rfunction{get.multimir}, you can type
\textit{?get.multimir} or \textit{help(get.multimir)} in R. In the next section,
...
It turns out that \textit{KRAS} is the only target validated by Luciferase
assay. The interaction was recorded in miRecords and miRTarBase and supported by
the same literature, whose PubMed ID is in column 'pubmed\_id'. The summary (by
setting \textit{"summary=TRUE"} when calling \Rfunction{get.multimir}) shows the
number of records in each of the external databases and the total number of
databases supporting the interaction.

\subsection{Example 2: Retrieve miRNA-target interactions associated with a
given drug or disease}
...
\Rfunction{get.multimir} returns 53 miRNA-target pairs. For more information, we
...
\Rfunction{get.multimir} also takes target gene(s) as input. In this example we
retrieve miRNAs predicted to target \textit{Gnb1} in mouse. For predicted
...
\textit{"predicted.cutoff = 20"} and \textit{"predicted.cutoff.type = 'p'"} (for
...
The records in \textit{example3\$predicted} are ordered by scores from best to
...
\subsection{Example 4: Select miRNA(s) predicted to target most, if not all, of
...
\begin{scriptsize}
<<Example4_part1>>=
example4 = get.multimir(org='hsa', target=c('AKT2','CERS6','S1PR3','SULF2'),
                        table='predicted',
...
\subsection{Example 5: Retrieve interactions between a set of miRNAs and a set
of genes}
...
Variable \textit{DE.miRNA.up} contains 9 up-regulated miRNAs and variable
\textit{DE.entrez.dn} has 47 down-regulated genes in the two metastatic cell
...
So we use \Rpackage{multiMiR} to check whether any of the nine miRNAs could
target any of the 47 genes.

\begin{scriptsize}
<<eval=FALSE, echo=TRUE>>=
# search all tables & top 10% predictions
example5 = get.multimir(org="hsa", mirna=DE.miRNA.up, target=DE.entrez.dn,
                        table="all",
...
As shown previously, \Rfunction{get.multimir} is the main function to retrieve
information from the \Rpackage{multiMiR} database, which is hosted at
...
advanced users, there are a couple ways to query the \Rpackage{multiMiR}
database without using the \Rpackage{multiMiR} package; but they have to be
familiar with SQL queries. In general, users are still advised to use the
\Rfunction{get.multimir} function when querying multiple external databases in
\Rpackage{multiMiR}.

\subsection{Direct query on the web server}

The \Rpackage{multiMiR} package communicates with the \Rpackage{multiMiR}
...
by typing \textit{multimir\_dbTables()} in R. Once again, data from each of the
external databases is stored in a table in \Rpackage{multiMiR}. There are also
tables for miRNAs (table \textit{mirna}) and target genes (table
\textit{target}).

\vspace*{1\baselineskip}
To learn about the structure of a table (e.g. DIANA-microT data in table
\textit{diana\_microt}), users can use URL
...
tables in \Rpackage{multiMiR}. While in Example 1, one line of R command using
the \Rfunction{get.multimir} function searches, combines and summarizes results
from all three validated external databases (miRecords, miRTarBase and TarBase).
...
These are equivalent to using the \Rfunction{multimir\_dbTables} function as
showed previously.
