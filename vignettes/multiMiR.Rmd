...
This example shows the use of `multiMiR` in a downstream Bioconductor workflow:
H:re we take ISS and ILS strain 


```{r, biocworkflow, eval=FALSE}
library(edgeR)
library(multiMiR)

counts_table   <- load(system.file("extdata", "counts_table.RData", 
                                   package = "multiMiR"))
strains_factor <- load(system.file("extdata", "strains_factor.RData", 
                                   package = "multiMiR"))
...
##   Getting top DE miRNA's
top_miRNAs <- rownames(p_val_DE_edgeR$table)[1:10]

##### UPSTREAM ######

####    Plug miRNA's into multiMiR and getting validated targets    ####
multimir_results <- get_multimir(org = 'mmu', mirna = top_miRNAs, table = 'validated', summary = T)
```
