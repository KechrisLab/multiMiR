...
[http://multimir.ucdenver.edu](http://multimir.ucdenver.edu), is a comprehensive
...
external databases in R without the need to visit all these databases.  Advanced
users can also submit SQL queries to the web server to retrieve results.  The
database is now versioned so it is possible to use previous versions of
databases from the current R package, however the package defaults to the most
recent version.
...
[http://multimir.ucdenver.edu](http://multimir.ucdenver.edu) hosts a database
...
multimir_switchDBVersion(db_version = "2.1.0")
```
...
associations. To check these categories and databases from within R,
we have a set of four helper functions:
...
*multiMiR* package also provides the `list.multimir()` function to list all the
...
```{r list.multimir, eval=FALSE}
miRNAs   = list.multimir("mirna", limit = 10)
genes    = list.multimir("gene", limit = 10)
drugs    = list.multimir("drug", limit = 10)
diseases = list.multimir("disease", limit = 10)
...
`get.multimir()` is the main function in the package to retrieve predicted and
...
`?get.multimir` or `help(get.multimir)` in R. In the next section, many examples
...
`summary = TRUE` when calling `get.multimir()`) shows the number of records in
...
`get.multimir()` returns 53 miRNA-target pairs. For more information, we can
...
`get.multimir()` also takes target gene(s) as input. In this example we retrieve
...
unique.pairs <- 
    unique(data.frame(miRNA.ID = as.character(example5$predicted$mature_mirna_id),
...
database via the script http://multimir.ucdenver.edu/cgi-bin/multimir_univ.pl on the
web server. Once again, data from each of the
...
query and dbName.  query- is the SQL query to submit.  dbName is the DBNAME column 
from a call to multimir_dbInfoVersions(), however if this is excluded the current 
...
http://multimir.ucdenver.edu/cgi-bin/multimir_univ.pl?query=describe diana\_microt
...
The same direct queries we did above on the web server can be done in R as well.  This
is the preferred method if you are unfamiliar with HTTP POST. Be sure to set the correct 
database version, if you wish to change versions, before calling 'search.multimir' it uses
...
direct3 <- search.multimir(query = qry)
direct3
```
