...
from an S3 (`mmquery`) to an S4 class (`mmquery_bioc`).  The new S4 object is
structured similar to the prior version. All features are now accessible using
the S4 accessor operator `@`. 

Additionally, the `AnnotationDbi` accessor methods `column`, `keys`, `keytypes`,
and `select` all work for `mmquery_bioc` objects.  See Section [annodbi].
...
multimir_results <- get_multimir(org     = 'mmu',
                                 ...
                                 table   = 'validated',
                                 summary = T)
head(multimir_results@validated)
```
...
select(example4, keys = mykeys, columns = c("database", "target_entrez"))
...
select(example3, keys = mykeys, columns = c("database", "target_entrez", "score"))

# Search by gene on example4's result
columns(example4)
keytypes(example4)
keys(example4, keytype = "target_entrez")
mykeys <- keys(example4, keytype = "target_entrez") [1:2]
select(example4, keys = mykeys, keytype = "target_entrez",
